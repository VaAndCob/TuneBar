...
void delete_screen_logo(lv_timer_t *timer) {
  ...
  // init widgets
  String txt = "BUILD  : " + version + " - " + compile_date + "\nSERIAL : " + String(ESP.getEfuseMac());
  lv_label_set_text(ui_MainMenu_Label_build, txt.c_str());
...
void previoustrack(lv_event_t *e) {
  ...
  case 1: { // music player mode
    ...
    String filePath = getTrackPath(trackIndex);
    log_d("Next track: %s", filePath.c_str());
    snprintf(status_buffer, sizeof(status_buffer), "%d of %d", trackIndex + 1, trackListLength);
    lv_label_set_text(ui_Player_Label_trackNumber, status_buffer);
    audioPlayFS(0, filePath.c_str());
    break;
  }
  case 2: // chatbot mode
...
void playpause(lv_event_t *e) {
  ...
  if (audio.isRunning()) {
    ...
    case 1: { // music player mode
      ...
      String filePath = getTrackPath(trackIndex);
      log_d("Play track: %s", filePath.c_str());
      snprintf(status_buffer, sizeof(status_buffer), "%d of %d", trackIndex + 1, trackListLength);
      lv_label_set_text(ui_Player_Label_trackNumber, status_buffer);
...
void hideArrow(lv_timer_t *timer) {
  ...
  arrowVisible = false;
  lv_obj_add_flag(ui_Info_Button_returnMenu2, LV_OBJ_FLAG_HIDDEN);
...
void showArrow(lv_event_t *e) {
  ...
  if (arrowVisible) return;
  arrowVisible = true;
  lv_obj_clear_flag(ui_Info_Button_returnMenu2, LV_OBJ_FLAG_HIDDEN);
